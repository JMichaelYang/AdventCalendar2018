...
public class Challenge04a implements IChallenge {
  ...
  public String[] translate(String[] input) {
    ...
    int id = getMaxId(guards);
    int minute = getMinute(guards.get(id));

    System.out.println("id: " + id + ", minute: " + minute);

    String[] ret = {String.valueOf(id * minute)};
    return ret;
  }

  int getMaxId(Map<Integer, Map<Integer, Integer>> guards) {
    int maxTimeAsleep = -1;
    int id = -1;
  ...
  int getMinute(Map<Integer, Integer> times) {
    int maxTimeAsleep = -1;
    int minute = -1;

    for (Integer i : times.keySet()) {
      if (times.get(i) > maxTimeAsleep) {
        maxTimeAsleep = times.get(i);
        minute = i;
      }
    }
